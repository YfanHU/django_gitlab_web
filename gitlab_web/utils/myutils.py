...
def show_account_status_for_user():
    ...
    content_str = ''.join([(dic['account_name'].join(contenter) + dic['availability'].join(contenter)).join(['<tr>','</tr>']) for dic in account_status_for_user()])
...
def get_uid_by_admin_username(admin_username):
    ...
    return sql_query_one('''select uid from account_sharing_admin_info where admin_username="{}"'''.format(admin_username))['uid']
